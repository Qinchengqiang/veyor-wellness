...
const ConfirmStep = props => {
    ...
    const confirmationSuccess = newBookingRedux.result === 'success' && newBookingRedux.data.id ?
        <>
            ...
            <p className='price mt-3 mb-3'>{`Veyor Wellness ${appointmentsRedux.data.find(item => item.name === newBookingRedux.data.appointment).price}`}</p>
